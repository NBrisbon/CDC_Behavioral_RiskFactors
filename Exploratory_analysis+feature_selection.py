...
model = LinearRegression()       #Initializing RFE model
rfe = RFE(model, 18)             #Transforming data using RFE
X_rfe = rfe.fit_transform(X1,y1)   #Fitting the data to model
model.fit(X_rfe,y1)              
...
cols = list(X1.columns)
model = LogisticRegression(solver='liblinear')       #Initializing RFE model
rfe = RFE(model, 12)             #Transforming data using RFE
X_rfe = rfe.fit_transform(X1,y2)   #Fitting the data to model
model.fit(X_rfe,y2)              
temp = pd.Series(rfe.support_,index = cols)
selected_features_rfe = temp[temp==True].index
...
# ### As you can see, it is difficult to select features for this set, given that all variables were originally chosen based on theory and previous research. These methods of feature selection would be more useful with a larger set of available features, particularly if there was no theory to use as guidance. 
# 
# ### This being the case, I'll use all current features from LLCP2 to choose a best model based on step-wise regression using a backwards elimination approach. 
# 
# ### I'm also going to stick to logistic regression, as it seems like it would be more straight-forward and beneficial to predict general risk for mental health issues, rather than to predict specifically how many days in a month someone might experience mental health issues.
